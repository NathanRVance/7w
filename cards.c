...
int* cards_getcost(int era, int card, int ret[NUMRESOURCES]) {
	...
	for (i = 0; i < NUMRESOURCES; i++)
		ret[i] = cards[era][card][STARTCOST + i];
	return ret;
}
...
void cards_setcoupons(int era, int card, int era1, int card1, int era2,
		int card2) {
	int i = STARTCOUPONS;
...
char* cards_getname(int era, int card, char ret[40]) {
	int i;
...
void cards_setmessage(int era, int card, char *message) {
	int i;
	for (i = 0; (cards[era][card][STARTDESCRIPTION + i] = message[i]) != '\0';
			i++)
		;
}
...
void cards_init() {
	...
	cards_setmessage(0, 18,
			"Can trade 1 coin for resources with player to the east.");
	cards_setname(0, 18, "East Trading Post", COMMERCIAL);

	cards_setcoupons(0, 19, 1, 11, 0, 0);
	cards_setmessage(0, 19,
			"Can trade 1 coin for resources with player to the west.");
	cards_setname(0, 19, "West Trading Post", COMMERCIAL);

	cards_setcoupons(0, 20, 1, 12, 0, 0);
	cards_setmessage(0, 20,
			"Can trade 1 coin for industry products with adjacent players.");
	...
	cards_setmessage(1, 13,
			"1 coin for each resource card of adjacent players or your own.");
	cards_setname(1, 13, "Vineyard", COMMERCIAL);

	cards_setmessage(1, 14,
			"2 coins for each industry card of adjacent players or your own.");
	...
	cards_setmessage(6, 5,
			"Can now play 7th age card rather than discarding it.");
	...
	cards_setmessage(7, 4,
			"Can trade 1 coin for resources with adjacent players.");
